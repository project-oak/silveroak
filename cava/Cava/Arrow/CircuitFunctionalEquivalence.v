...
(* (1* wrapper for circuit_equiv that keeps goals in standard, recursion-friendly form *1) *)
...
(* (1* this lemma helps get a subcircuit goal into the [obeys_spec] form so eauto *)
(*    recognizes it *1) *)
...
(* (1* Add [obeys_spec] proofs to this hint database, and circuit_spec will use *)
(*    them *1) *)
...
(*       (1* simplify combinational_evaluation' if there's a primitive *1) *)
(*       cbv [combinational_evaluation' primitive_interp] *)
(*     | |- _ = @resize_default _ ?n ?d ?n ?v => *)
(*       (1* change resize_default to identity function if it appears *1) *)
(*       transitivity v; [ | rewrite resize_default_id; reflexivity ] *)
(*     | |- _ = Vector.map (fun x => x) ?v => *)
(*       (1* change Vector.map id to identity function if it appears *1) *)
...
(*     lazymatch k with *)
(*     | @Abs _ ?x ?y ?z ?f => *)
(*       (1* sometimes the types need to be coerced for Abs *1) *)
...
(* (1* Version of [circuit_spec] that expects to solve the goal *1) *)
