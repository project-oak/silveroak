...
| IndexArray: forall n xs,
  structure (
    Tuple2 (One (BitVec (n::xs)))
      (Tuple2 (One (BitVec [PeanoNat.Nat.log2_up n]))
      Empty)
  )
  (One (BitVec xs))
| ConvertDegenerateBitVec:
  structure (
    Tuple2 (One (BitVec []))
      Empty)
    (One Bit)
...
Lemma denoteNobj_is_replicate_object `{Cava}: forall n, denoteShape (Nobj n Bit) = replicate_object n bit.
Proof.
  intros.
  induction n.
  auto.
...
Fixpoint toCava {i o} (Cava:Cava) (expr: structure i o)
  ...
  | Lut n f => _
  | IndexArray n xs => index_array n xs
  | ConvertDegenerateBitVec => convert_degenerate_bitvec
  end).
  rewrite denoteNobj_is_replicate_object.
  simpl denoteShape.
  exact (lut n f).
Defined.
...
#[refine] Instance ConstructiveCava : Cava := {
  ...
  convert_degenerate_bitvec := ConvertDegenerateBitVec;
}.
Proof.
  intros.
  simpl.
