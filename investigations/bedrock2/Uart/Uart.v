...
Section Impl.
  ...
  (****
     static void uart_reset(void) {
       WRITE(TOP_EARLGREY_UART0_BASE_ADDR + UART_CTRL_REG_OFFSET, 0u);
       ...
       WRITE(TOP_EARLGREY_UART0_BASE_ADDR + UART_FIFO_CTRL_REG_OFFSET,
     reg);

       WRITE(TOP_EARLGREY_UART0_BASE_ADDR + UART_OVRD_REG_OFFSET, 0u);
       WRITE(TOP_EARLGREY_UART0_BASE_ADDR + UART_TIMEOUT_CTRL_REG_OFFSET,
     0u);
       WRITE(TOP_EARLGREY_UART0_BASE_ADDR + UART_INTR_ENABLE_REG_OFFSET,
     0u);
       WRITE(TOP_EARLGREY_UART0_BASE_ADDR + UART_INTR_STATE_REG_OFFSET,
     UINT32_MAX);
     }
   ***)
  Definition uart_reset : func :=
    ...
    ("b2_uart_reset", ([], [],
    bedrock_func_body:(
      output! WRITE(TOP_EARLGREY_UART0_BASE_ADDR + UART_CTRL_REG_OFFSET, 0);
      ...
      output! WRITE(TOP_EARLGREY_UART0_BASE_ADDR + UART_FIFO_CTRL_REG_OFFSET, reg);

      output! WRITE(TOP_EARLGREY_UART0_BASE_ADDR + UART_OVRD_REG_OFFSET, 0);
      output! WRITE(TOP_EARLGREY_UART0_BASE_ADDR + UART_TIMEOUT_CTRL_REG_OFFSET, 0);
      output! WRITE(TOP_EARLGREY_UART0_BASE_ADDR + UART_INTR_ENABLE_REG_OFFSET, 0);
      (* -1 instead of UINT32_MAX *)
      output! WRITE(TOP_EARLGREY_UART0_BASE_ADDR + UART_INTR_STATE_REG_OFFSET, -1)
    ))).
  ...
  (****
     rom_error_t uart_init(uint32_t precalculated_nco) {
       ...
       WRITE(TOP_EARLGREY_UART0_BASE_ADDR + UART_CTRL_REG_OFFSET, reg);
    ...
    bedrock_func_body:(
      if (precalculated_nco == 0) {
        ...
        if ((precalculated_nco & (UART_CTRL_NCO_MASK^(-1))) == 0) {
          ...
          output! WRITE(TOP_EARLGREY_UART0_BASE_ADDR + UART_CTRL_REG_OFFSET, reg);
          output! WRITE(TOP_EARLGREY_UART0_BASE_ADDR + UART_INTR_ENABLE_REG_OFFSET, 0);
          out = kErrorOk
        } else {
          out = kErrorUartInvalidArgument
  ...
  (****
    static bool uart_tx_full(void) {
      uint32_t reg =
          READ(TOP_EARLGREY_UART0_BASE_ADDR + UART_STATUS_REG_OFFSET);
  ...
  Definition uart_tx_full : func :=
    ...
    ("b2_uart_tx_full", ([], [out],
    bedrock_func_body:(
      io! reg = READ(TOP_EARLGREY_UART0_BASE_ADDR + UART_STATUS_REG_OFFSET);
  ...
  (****
    void uart_putchar(uint8_t byte) {
      ...
      WRITE(TOP_EARLGREY_UART0_BASE_ADDR + UART_WDATA_REG_OFFSET, reg);
  ...
  Definition uart_putchar : func :=
    ...
    bedrock_func_body:(
      ...
      output! WRITE(TOP_EARLGREY_UART0_BASE_ADDR + UART_WDATA_REG_OFFSET, reg);
