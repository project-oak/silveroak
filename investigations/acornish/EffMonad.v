...
Section IdentInstance.
  Context {x : Type}.
  Context {eff: Monoid x}.

  Inductive Ident (st: x) T := mkIdent { runIdent : T }.

  Instance ident_eff_monad: EffMonad (x:=x) (eff:=eff) Ident :=
  { ret _ x := mkIdent _ _ x
  ; bind _ _ _ _ x f :=
    mkIdent _ _ (
      match runIdent _ _ x with
      | x => fun f => runIdent _ _ (f x)
      end f)
  }.
End IdentInstance.

Section FuelInstance.
  Context {x : Type}.
  Context {eff: Monoid x}.
  ...
  Inductive Fuelled T := mkFuelled { runFuelled : nat -> FixResult T }.

  Instance fuelled_eff_monad: EffMonad (x:=x) (eff:=eff) (fun _ => Fuelled) :=
  { ret _ x := mkFuelled _ (fun _ => Thunk x)
  ; bind _ _ _ _ x f :=
    mkFuelled _ (fun n =>
      match runFuelled _ x n with
      | Thunk x => fun f => runFuelled _ (f x) n
