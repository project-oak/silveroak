...
Section WithCava.
  ...
  Definition sub_round (a b: signal round_index): cava (signal round_index) :=
    b <- peel b ;;
    b <- Traversable.mapT inv b ;;
    b <- unpeel b ;;
    b <- inc_round b ;;
    sum <- (@unsignedAdd _ _ 4 4 (a, b)) ;;
    sum <- peel sum ;;
  ...
  Definition transition_round (inputs: control_inputs): signal_update :=
    ...
    (*   r2 <- round_2 ;; *)
    (*   num_rounds_regular <- sub_round num_rounds_q r2 ;; *)
    (*   round_q <- round_q ;; *)
  ...
  Definition aes_cipher_core
    ...
    Loop (
      ...
      Comb (fun inputs =>
        ...
        key_full_mux <- mux4 (input_kp, input_kp, kp, input_kp) key_full_sel ;;
        ...
        (* is_r0 <- eqb r0 round ;; (1* bugged? *1) *)
        is_r0 <- eqb_bb  (r0, round) ;;
        k <- mux2 is_r0 (k1, k0);;
        ...
        ret
          ...
          , ( in_ready, out_valid, crypt, dec_key_gen, key_clear, data_out_clear
            , k, st, round, r0, is_r0, key_full_mux))
