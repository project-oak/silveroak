...
Section Var.
  ...
  Section Cava.
    ...
    Definition circuit_state {i s o} (c : Circuit s i o) := denote_type s.
  End Cava.
...
Definition incr_device_step :
  ...
  denote_type [Bit; Bit; BitVec 32; BitVec 32] ->
  (_
   (* output: next state, is_resp, resp *)
   * denote_type (Bit ** BitVec 32))
  := step incr.

Require Import coqutil.Datatypes.List.

(* (* like `simulate`, but also output the internal state, for more insightful debugging *) *)
(* Fixpoint simulate_with_state_rec {o} *)
(*          (* (c : Circuit i o) *) *)
(*          c *)
(*          (* (initial : circuit_state c) *) *)
(*          initial *)
(*          (inps : list type) *)
...
(*   match inps with *)
(*   | [] => [] *)
(*   | inp :: rest => let r := step c initial (denote_type inp) in *)
(*                  r :: simulate_with_state_rec c (fst r) rest *)
(*   end *)
(* . *)
...
Section WithParameters.
  ...
  Definition mk_counter_state(idle busy done : bool)(val : N) : circuit_state incr :=
    (idle, (busy, (done, val))).
  ...
  | BUSY_related: forall val ncycles,
      (0 < ncycles)%nat ->
      counter_related (BUSY val ncycles) (mk_counter_state false true false (word_to_N val))
  ...
  | BUSY_done_related: forall val ncycles,
      counter_related (BUSY val ncycles)
                      (mk_counter_state false false true (word_to_N (word.add (word.of_Z 1) val)))
  | DONE_related: forall val,
      counter_related (DONE val) (mk_counter_state false false true (word_to_N val)).

  Axiom TODO: Coq.Init.Logic.False.

  Axiom word_to_N_inj: forall x y, word_to_N x = word_to_N y -> x = y.

  Axiom incrN_word_to_bv: forall v, N.add (word_to_N v) 1 = word_to_N (word.add (word.of_Z 1) v).
